...
public class dbConnectivity 
{
    ...
    public dbConnectivity() //cons
    {
        try
        {
           ...
           // String s = "jdbc:sqlserver://DESKTOP-DBTI71B//SQLEXPRESS;databaseName=Donation";System.setProperty("java.net.preferIPv6Addresses", "true");  

String s ="jdbc:sqlserver://<hostname>:<1433>;instanceName=<DESKTOP-DBTI71B\\SQLEXPRESS>;databaseName=<Donation>";
             System.setProperty("java.net.preferIPv6Addresses", "true"); 
//"DESKTOP-DBTI71B\\SQLEXPRESS"
             ////sssscon=DriverManager.getConnection(s,"sa","12345678");
            con=DriverManager.getConnection(s,"ghassan","ghassan");
    ...
    public int addDonation(Donation d)
    {
     int xx=0; 
        ...
        public String getbeneficiaryname (String cnic)
        {
         ...
         public ArrayList<String> getprojectbeneficiaryname (String project)
        {
            ArrayList <String> name=new ArrayList<>();
            try{
           ResultSet rs = stmt.executeQuery("select Beneficiary from beneficiaryprojects where project like '"+project+"'");
           while(rs.next())
           {
        ...
        public ArrayList<String> getreplacemanagernames (String n)
        {
            ArrayList <String> name=new ArrayList<>();
            try{
           ResultSet rs = stmt.executeQuery("select nam from volunteer where avail = 1 and nam not like '"+n+"'");
           while(rs.next())
           {
               name.add(rs.getString(1));
           }
        ...
        public String getmanagername (String n)
        {
            String name=null;
            try{
           ResultSet rs = stmt.executeQuery("select manager from project where nam like '"+n+"'");
           while(rs.next())
           {
               name=(rs.getString(1));
           }
        ...
        public ArrayList <String> getcurrentvolunteers (String n)
        {
            ArrayList <String> name=new ArrayList <>();
            try{
           ResultSet rs = stmt.executeQuery("select volunteer from team where project like '"+n+"'");
           while(rs.next())
           {
               name.add(rs.getString(1));
           }
        ...
        public ArrayList <String> getremainingvolunteers (String n)
        {
            ArrayList <String> name=new ArrayList <>();
            try{
           ResultSet rs = stmt.executeQuery("select nam from volunteer where avail = 1 except select volunteer from team where project like '"+n+"'");
           while(rs.next())
           {
               name.add(rs.getString(1));
           }
        ...
        public int removevolunteer(String name,String project)
        {
           ...
           try
            {
            CallableStatement cstmt = con.prepareCall("{call removevolunteer(?,?)}");
        ...
        public int replacevolunteer(String name,String name2,String project)
        {
           ...
           try
            {
            CallableStatement cstmt = con.prepareCall("{call replacevolunteer(?,?,?)}");
            cstmt.setString(1, name);
            cstmt.setString(2, name2);
            cstmt.setString(3, project);
            cstmt.execute();  
            }
        catch(SQLException e)
            {
        ...
        public int replacemanager(String name,String project)
        {
           ...
           try
            {
            CallableStatement cstmt = con.prepareCall("{call replacemanager(?,?)}");
            cstmt.setString(1, name);
            cstmt.setString(2, project);
            cstmt.execute();  
            }
       ...
       public int addissuedonation (String project , String beneficiary , int amount , int weight)
       {
           ...
           try
            {
            CallableStatement cstmt = con.prepareCall("{call addissuedonation(? , ? , ? , ?)}");
            cstmt.setString(1, project);
            cstmt.setString(2, beneficiary);
            cstmt.setInt(3 , amount);
            cstmt.setInt(4,weight);
            cstmt.execute();  
            }
        catch(SQLException e)
            {
       ...
       public int[] checkbalance(String project)
       {
        ...
        try
        {
            CallableStatement cstmt = con.prepareCall("{call checkbalance(? , ? , ?)}");
            cstmt.setString(1, project);
            cstmt.registerOutParameter(2, java.sql.Types.INTEGER); 
            cstmt.registerOutParameter(3, java.sql.Types.INTEGER); 
            cstmt.execute();  
            arr[0]=cstmt.getInt(2);  
            arr[1]=cstmt.getInt(3);
        }
        catch(SQLException e)
        {
         e.printStackTrace();
