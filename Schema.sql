...
BEGIN
declare @x int,@y int,@x1 int,@y1 int
select @amount  = (select sum(amount) from donation where project = @project and dontype = 'cash') - (select sum(amount) from issuedonations where project = @project)
select @weight = (select sum(amount) from donation where project = @project and dontype = 'wheat') - (select sum(weight) from issuedonations where project=@project)
END;
